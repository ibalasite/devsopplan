...
# VERSION: 2.4.0
# Auto-generated by devsop-gen-html — run anytime to regenerate all HTML pages
...
PAGE_META = {
    ...
    'schema':           ('Schema 文件',              '🗄️'),
    'requirements':     ('需求規格',                 '📌'),
    'migration_tracker':('Migration Tracker',        '🔄'),
    'product_review':   ('Product Review',           '🔍'),
    'solution_roadmap': ('Solution Roadmap',         '🗺️'),
    'bdd':              ('BDD Scenarios',            '🧪'),
}
...
def main():
    ...
    # Build page list: index first, then known order, then extras
    known_order = ['PRD', 'BRD', 'PDD', 'EDD', 'ARCH', 'API', 'SCHEMA',
                   'REQUIREMENTS', 'MIGRATION_TRACKER', 'PRODUCT_REVIEW', 'SOLUTION_ROADMAP']
    pages = [('index', '首頁', '🏠')]
    doc_pages = []
    known_slugs = {'readme'}
    ...
    for name in known_order:
        ...
        if not p.exists():
            ...
            for f in DOCS_DIR.glob("*.md"):
                if f.stem.upper() == name:
                    p = f
                    break
        if p.exists():
            s = p.stem.lower()
            label, icon = PAGE_META.get(s, (p.stem.replace('_', ' ').title(), '📄'))
    ...
    has_bdd = FEATURES_DIR.exists() and list(FEATURES_DIR.glob("*.feature"))
    if has_bdd:
    ...
    # Known + extra doc pages
    for s, label, p in doc_pages + extra_pages:
